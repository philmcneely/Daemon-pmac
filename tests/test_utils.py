...
from app.utils import (
    create_backup, cleanup_old_backups, health_check, get_uptime,
    is_single_user_mode, get_single_user
)
...
def test_is_single_user_mode(test_db_session):
    ...
    user2 = User(username="second_user", email="second@test.com", hashed_password="hash")
...
def test_get_single_user(test_db_session):
    ...
    user2 = User(username="admin_user", email="admin@test.com", hashed_password="hash", is_admin=True)
    test_db_session.add(user2)
    test_db_session.commit()
    single_user = get_single_user(test_db_session)
