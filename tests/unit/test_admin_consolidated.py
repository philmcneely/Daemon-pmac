...
class TestAdminApiKeyManagement:
    ...
    def test_create_api_key_success(self, mock_generate_api_key):
        ...
        mock_generate_api_key.return_value = ("test-api-key-value", "test-hash")
        ...
        def mock_get_db():
            ...
            # Mock the db.refresh to set the attributes (original working pattern)
...
class TestAdminEndpointManagement:
    ...
    def test_toggle_endpoint_success(self):
        ...
        try:
            ...
            assert response.status_code == 200
            data = response.json()
            assert data["message"] == "Endpoint deactivated"
        finally:
            app.dependency_overrides.clear()
    ...
    def test_delete_endpoint_success(self):
        ...
        try:
            ...
            assert response.status_code == 200
            data = response.json()
            assert "Endpoint 'books' and all its data deleted" in data["message"]
        finally:
            app.dependency_overrides.clear()
...
class TestAdminBackupOperations:
    ...
    def test_restore_database_success(self, mock_exists, mock_copy):
        ...
        try:
            ...
            assert response.status_code == 200
            data = response.json()
            assert "Database restored from" in data["message"]
        finally:
            app.dependency_overrides.clear()
    ...
    def test_sql_injection_attempts(self, unit_client):
        ...
        for payload in sql_payloads:
            ...
            assert response.status_code in [
                ...
                404,
                422,
            ]  # 404 is valid for invalid user ID
