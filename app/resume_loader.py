...
def import_resume_to_database(
    ...
    try:
        ...
        resume_endpoint = db.query(Endpoint).filter(
            Endpoint.name == RESUME_ENDPOINT_NAME,
...
def get_resume_from_database() -> Dict[str, Any]:
    ...
    try:
        ...
        resume_entries = db.query(DataEntry).filter(
            DataEntry.endpoint_id == resume_endpoint.id,
            DataEntry.is_active == True
        ).all()
        ...
        return {
            ...
            "entries": [{"id": entry.id, "created_at": entry.created_at} for entry in resume_entries]
        }
