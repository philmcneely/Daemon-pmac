...
from app.utils import create_backup, cleanup_old_backups, export_endpoint_data, import_endpoint_data
    ...
    try:
        ...
        if file_info['exists']:
            ...
            if load_result['success']:
                ...
                click.echo(f"  Experience entries: {len(resume_data.get('experience', []))}")
                click.echo(f"  Education entries: {len(resume_data.get('education', []))}")
            else:
        ...
        if result['success']:
            if result['count'] == 0:
                ...
                for i, entry_info in enumerate(result.get('entries', [])):
                    click.echo(f"  Entry {i+1}: ID {entry_info['id']}, Created: {entry_info['created_at']}")
                ...
                if result['data']:
                    ...
                    click.echo(f"  Experience: {len(resume.get('experience', []))} entries")
                    click.echo(f"  Education: {len(resume.get('education', []))} entries")
...
def list():
    ...
    try:
        ...
        click.echo(f"{'ID':<5} {'Username':<20} {'Email':<30} {'Active':<8} {'Admin':<8}")
        click.echo("-" * 80)
...
def list():
    ...
    try:
        ...
        click.echo(f"{'ID':<5} {'Name':<20} {'Description':<40} {'Active':<8} {'Public':<8}")
        click.echo("-" * 90)
        
        for ep in endpoints:
            desc = (ep.description or "")[:37] + "..." if (ep.description or "") and len(ep.description) > 40 else (ep.description or "")
            click.echo(f"{ep.id:<5} {ep.name:<20} {desc:<40} "
...
def restore(backup_filename):
    ...
    if not click.confirm(f"This will overwrite the current database with '{backup_filename}'. Continue?"):
        return
...
@click.option('--format', default='json', type=click.Choice(['json', 'csv']), help='Import format')
    ...
    try:
        ...
        if result['errors']:
            ...
            for error in result['errors'][:5]:  # Show first 5 errors
                click.echo(f"  - Item {error['index']}: {error.get('error', error.get('errors', []))}")
...
def status():
    ...
    try:
        ...
        for check_name, check_result in health['checks'].items():
            status_icon = {"healthy": "✓", "degraded": "⚠", "unhealthy": "✗", "disabled": "-", "warning": "⚠", "critical": "✗", "unknown": "?"}.get(check_result['status'], "?")
...
def create_user(username: str, email: str, password: str, admin: bool, import_data: bool):
    ...
    try:
        ...
        if import_data:
            ...
            if result["success"]:
                click.echo(f"✓ Imported {result['total_entries']} data entries from {len(result['imported_files'])} files")
            else:
                click.echo(f"⚠ Data import failed: {result.get('error', 'Unknown error')}")
...
@click.command()
@click.option('--base-dir', default='data/private', help='Base directory containing user folders')
...
def import_all_data(base_dir: str, replace: bool):
    ...
    try:
        ...
        if result["success"]:
            click.echo(f"✓ Successfully imported data for {result['total_users']} users")
            click.echo(f"✓ Total entries imported: {result['total_entries']}")
            
            for user_result in result["users_processed"]:
                click.echo(f"  - {user_result['username']}: {user_result['total_entries']} entries")
    ...
    try:
        ...
        if result["success"]:
            ...
            for file_info in result["imported_files"]:
                click.echo(f"  - {file_info['endpoint']}: {file_info['entries']} entries from {file_info['file']}")
