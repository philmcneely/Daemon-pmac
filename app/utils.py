    ...
    for field_name, field_schema in schema.items():
        ...
        if field_type == "string" and isinstance(value, str):
            if "min_length" in field_schema and len(value) < field_schema["min_length"]:
                errors.append(f"Field '{field_name}' must be at least {field_schema['min_length']} characters")
            if "max_length" in field_schema and len(value) > field_schema["max_length"]:
                errors.append(f"Field '{field_name}' must be at most {field_schema['max_length']} characters")
        
        # Number range validation
        if field_type in ["integer", "number"] and isinstance(value, (int, float)):
            if "minimum" in field_schema and value < field_schema["minimum"]:
                errors.append(f"Field '{field_name}' must be at least {field_schema['minimum']}")
            if "maximum" in field_schema and value > field_schema["maximum"]:
                errors.append(f"Field '{field_name}' must be at most {field_schema['maximum']}")
...
def export_endpoint_data(db_session, endpoint_name: str, format: str = "json") -> str:
    ...
    # Find endpoint
    endpoint = db_session.query(Endpoint).filter(
    ...
    # Get data
    data_entries = db_session.query(DataEntry).filter(
...
def health_check() -> Dict[str, Any]:
    ...
    try:
        if settings.backup_enabled:
            if os.path.exists(settings.backup_dir) and os.access(settings.backup_dir, os.W_OK):
                health["checks"]["backup_dir"] = {"status": "healthy", "message": "Backup directory accessible"}
            else:
                health["status"] = "degraded"
                health["checks"]["backup_dir"] = {"status": "degraded", "message": "Backup directory not accessible"}
        else:
            health["checks"]["backup_dir"] = {"status": "disabled", "message": "Backup functionality disabled"}
    except Exception as e:
        health["status"] = "degraded"
    ...
    try:
        ...
        if free_percent < 10:
            ...
            health["status"] = "unhealthy"
            health["checks"]["disk_space"] = {"status": "critical", "message": f"Critical disk space: {free_percent:.1f}% free"}
        else:
            health["checks"]["disk_space"] = {"status": "healthy", "message": f"Disk space: {free_percent:.1f}% free"}
    except Exception as e:
...
def sanitize_input(value: Any) -> Any:
    ...
    if isinstance(value, str):
        ...
        url_pattern = r'[a-zA-Z][a-zA-Z0-9+.-]*://[^\s]+'  # More comprehensive URL pattern
...
def mask_sensitive_data(data: Dict[str, Any], level: str = "business_card") -> Dict[str, Any]:
    """
    Mask sensitive data based on privacy level
    """
    sensitive_fields = {
        "business_card": ["ssn", "credit_card", "password", "api_key", "private_key", "secret"],
    ...
    def recursively_mask(obj):
        ...
        if isinstance(obj, dict):
            ...
            for key, value in obj.items():
                ...
                should_mask = any(sensitive_field.lower() in key.lower() for sensitive_field in fields_to_mask)
                # Debug: print what we're checking
                # print(f"Checking key '{key}' against fields {fields_to_mask}, should_mask: {should_mask}")
                if should_mask:
                    masked_obj[key] = "***REDACTED***"  # Mask all sensitive fields regardless of type
                else:
...
def validate_url(url: str) -> bool:
    ...
    dangerous_protocols = [
        'file://', 'ftp://', 'gopher://', 'dict://', 'jar://', 'netdoc://',
        'javascript:', 'data:', 'vbscript:', 'jar:', 'phar://'
    ]
