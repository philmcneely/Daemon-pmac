...
class PrivacyFilter:
    ...
    def __init__(self, db: Session, user: Optional[User] = None):
        ...
        if user:
            self.privacy_settings = db.query(UserPrivacySettings).filter(
                UserPrivacySettings.user_id == user.id
            ).first()
    
    def filter_data(self, data: Dict[str, Any], privacy_level: str = "public", is_authenticated: bool = False) -> Dict[str, Any]:
        """
    ...
    def ai_safe_filter(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sensitive_for_ai = {
            "current_location", "home_address", "phone", "mobile",
            "personal_email", "family", "relationships", "medical",
            "financial", "legal", "private_notes", "internal_notes"
        }
    ...
    def professional_filter(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        professional_excludes = {
            "personal", "family", "relationships", "medical", "health",
            "financial", "salary", "compensation", "bank", "tax",
            "home_address", "personal_phone", "emergency_contact"
        }
    ...
    def _apply_user_privacy_settings(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not settings.show_contact_info and "contact" in filtered_data:
            ...
            essential = {}
            if "email" in contact and not any(word in contact["email"].lower() 
        ...
        if not settings.show_current_company and "experience" in filtered_data:
            if isinstance(filtered_data["experience"], list) and filtered_data["experience"]:
        ...
        if settings.custom_privacy_rules:
            filtered_data = self._apply_custom_rules(filtered_data, settings.custom_privacy_rules)
    ...
    def _apply_sensitive_patterns(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sensitive_patterns = {
            ...
            "phone", "mobile", "cell", "telephone", "personal_phone", "home_phone",
            "emergency_contact", "emergency_phone", "emergency",
            "personal_email", "private_email", "home_email",
            "address", "home_address", "street", "apartment", "apt", "suite",
            "zip", "zipcode", "postal_code", "postcode",
            
            # Financial info
            "salary", "wage", "compensation", "income", "pay", "earnings",
            "bank", "account", "routing", "iban", "swift", "credit_card",
            "tax_id", "ssn", "social_security", "ein",
            
            # Government IDs
            "passport", "drivers_license", "dl_number", "license_number",
            "visa", "green_card", "citizenship",
            
            # Sensitive flags
            "private", "confidential", "internal", "secret", "restricted",
            "intimate", "family_only",
            
            # Medical/Health
            "medical", "health", "diagnosis", "prescription", "allergy",
            "disability", "mental_health",
            
            # Security
            "password", "pin", "security_question", "recovery", "backup_codes",
            "api_key", "token", "secret_key", "auth"
        }
        ...
        def is_sensitive_value(value):
            if isinstance(value, str):
                ...
                if len(phone_cleaned) >= 10 and re.match(r'^[\+]?[\d\s\-\(\)\.]{10,}$', value):
                    return True
                # SSN pattern
                if re.match(r'\d{3}-?\d{2}-?\d{4}', value):
                    return True
                # Personal email indicators
                if '@' in value and any(word in value.lower() 
                                      for word in ['personal', 'private', 'home']):
                    return True
            return False
        ...
        def recursive_filter(obj, path=""):
            if isinstance(obj, dict):
                ...
                for key, value in obj.items():
                    ...
                    filtered_value = recursive_filter(value, f"{path}.{key}" if path else key)
                ...
                return [recursive_filter(item, f"{path}[{i}]") 
    ...
    def _recursive_filter_fields(self, data: Dict[str, Any], exclude_fields: set) -> Dict[str, Any]:
        """Recursively filter out specific field names"""
        def filter_recursive(obj):
            if isinstance(obj, dict):
                return {
                    k: filter_recursive(v) for k, v in obj.items()
    ...
    def _apply_custom_rules(self, data: Dict[str, Any], custom_rules: Dict[str, Any]) -> Dict[str, Any]:
