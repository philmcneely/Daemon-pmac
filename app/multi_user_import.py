...
def import_user_data_from_directory(
    ...
    endpoint_dirs = ["resume", "skills", "books", "hobbies", "ideas", "problems", "looking_for"]
    ...
    for file_path in json_files:
        try:
            ...
            endpoint_name = filename.replace(f"_{username}", "").replace(f"_{user.username}", "")
...
def import_user_file(
    ...
    for item in data_items:
        try:
            ...
            return {
                "success": False,
                "error": f"Failed to create data entry: {str(e)}"
            }
...
def import_all_users_data(
    ...
    user_directories = [d for d in os.listdir(base_directory) 
...
def create_user_data_directory(username: str, base_directory: str = "data/private") -> str:
    """
