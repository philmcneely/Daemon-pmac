...
def insert_default_privacy_rules():
    ...
    with engine.connect() as conn:
        ...
        default_rules = [
            ...
            ("email", r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b", "professional"),
            ...
            ("address", r"\d+\s+[A-Za-z0-9\s,.-]+\s+(Street|St|Avenue|Ave|Road|Rd|Drive|Dr|Lane|Ln)", "professional"),
            ...
            ("dob_alt", r"\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},?\s+\d{4}\b", "professional"),
        ]
...
def create_default_privacy_settings_for_existing_users():
    ...
    with engine.connect() as conn:
        ...
        print(f"Creating default privacy settings for {len(users_without_settings)} users...")
...
def main():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            settings_count = conn.execute(text("SELECT COUNT(*) FROM user_privacy_settings")).scalar()
