...
def create_test_content(token):
    ...
    for name, content in [
        ...
        if response.status_code == 200:
            ...
            print(
                f"❌ Failed to create {name} about content: {response.status_code} {response.text}"
            )
        ...
        if response1.status_code == 200 and response2_follow.status_code == 200:
            try:
                ...
                if data1 == data2:
                    ...
                    print(
                        f"      Pattern 1 items: {len(data1) if isinstance(data1, list) else 'not a list'}"
                    )
                    print(
                        f"      Pattern 2 items: {len(data2) if isinstance(data2, list) else 'not a list'}"
                    )
